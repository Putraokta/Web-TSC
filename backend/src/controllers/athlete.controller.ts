...
export default {
    async create(req: IAuthRequest, res: Response) {
        try {
            ...
            // map incoming schoolIds -> schools
    ...
    async findAll(req: IAuthRequest, res: Response) {
        ...
        try {
            ...
            pagination(res, "Successfully retrieved athletes", { total: count, totalPages: Math.ceil(count / limit), currentPage: Number(page) }, results);
        } catch (err) {
    ...
    async update(req: IAuthRequest, res: Response) {
        try {
            ...
            const result = await AthleteModel.findByIdAndUpdate(id, req.body, { new: true, runValidators: true });
            ...
            if (err instanceof Error && "code" in err && err.code === 11000) {
                return error(res, { message: "Duplicate key error"}, "Failed to update athlete due to duplicate key");
            }
        }
    },
    ...
    async remove(req: IAuthRequest, res: Response) {
        try {
            const { id } = req.params;
            const result = await AthleteModel.findByIdAndUpdate(id, { isActive: false }, { new: true });
